...
<body>
  ...
  <div class="page-content">
    <style>
      ...
      .function-card p, .function-card ul {
        color: #8a9aaa;
      }
      .function-card code {
      ...
      .try-form input::placeholder, .try-form textarea::placeholder {
        color: rgba(122, 138, 153, 0.6);
      }
      ...
      .try-form button {
        background: linear-gradient(135deg, #0ca0b1 0%, #087a8c 100%);
        border: none;
        color: #0a0f16;
        font-weight: 600;
      }
      ...
      .result-box {
        background: rgba(6, 11, 20, 0.7);
        border: 1px solid rgba(0, 155, 155, 0.15);
        border-radius: 8px;
        padding: 1rem;
        color: #7a8a99;
        margin-top: 1rem;
    ...
    <div class="container-fluid p-5">
      <div class="function-card">
        <h2><i class="bi bi-gear me-2"></i>CASE Expression</h2>
        
        <h3>Description</h3>
        <p>The <code>CASE</code> expression returns a value based on one or more conditions. It works similar to if-then-else statements in programming languages.</p>
...
END</code></pre>
        ...
        <ul style="color: #e6eef3;">
          <li>The corresponding result value when a condition is true</li>
          <li>The ELSE value if no conditions are true</li>
        </ul>
      </div>
      ...
      <div class="function-card">
        ...
        <table class="table example-table">
          ...
          <tbody>
            ...
            $result = $conn->query("SELECT Name, Department, 
              CASE 
                WHEN Salary >= 35000 THEN 'High'
                WHEN Salary >= 28000 THEN 'Medium'
                ELSE 'Low'
            ...
            while ($row = $result->fetch_assoc()) {
              ...
              echo "<td><code>" . htmlspecialchars($row['salary_category']) . "</code></td>";
              echo "</tr>";
            }
            ?>
          </tbody>
      ...
      <div class="function-card">
        ...
        <p>Create a CASE expression to categorize salary ranges:</p>
        <form method="POST" class="try-form">
          <div class="input-group mb-3">
            <input type="number" class="form-control" name="case_salary" placeholder="Enter salary amount" value="<?php echo isset($_POST['case_salary']) ? htmlspecialchars($_POST['case_salary']) : ''; ?>" required>
            <button class="btn" type="submit">Check Category</button>
          </div>
        </form>
        
        <?php
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['case_salary'])) {
            $salary = intval($_POST['case_salary']);
            $result = $conn->query("SELECT CASE 
              WHEN {$salary} >= 35000 THEN 'High Salary'
              WHEN {$salary} >= 28000 THEN 'Medium Salary'
              ELSE 'Low Salary'
            END as category");
            $row = $result->fetch_assoc();
            ?>
            <div class="result-box">
              <strong>Result:</strong> Salary {$salary} is categorized as: <code><?php echo htmlspecialchars($row['category']); ?></code>
            </div>
            <?php
        }
        $conn->close();
