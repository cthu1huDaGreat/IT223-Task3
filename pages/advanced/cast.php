...
<body>
  ...
  <div class="page-content">
    <style>
      ...
      .function-card pre { background: rgba(6, 11, 20, 0.9); border: 1px solid rgba(0, 229, 255, 0.08); padding: 1rem; border-radius: 8px; color: #0ca0b1; overflow-x: auto; }
    ...
    <div class="container-fluid p-5">
      <div class="function-card">
        <h2><i class="bi bi-gear me-2"></i>CAST() Function</h2>
        <h3>Description</h3>
        <p>The <code>CAST()</code> function converts a value from one datatype to another.</p>
        <h3>Syntax</h3>
        <pre><code>CAST(expr AS type)</code></pre>
      </div>
      <div class="function-card">
        <h3>Database Examples</h3>
        <table class="table example-table">
          <thead><tr><th>Salary</th><th>As CHAR</th></tr></thead>
          <tbody>
            <?php include '../../config.php'; $result = $conn->query("SELECT Salary, CAST(Salary AS CHAR) as char_val FROM employee LIMIT 3");
            while ($row = $result->fetch_assoc()) { echo "<tr><td>" . $row['Salary'] . "</td><td><code>" . htmlspecialchars($row['char_val']) . "</code></td></tr>"; } $conn->close(); ?>
          </tbody>
        </table>
      </div>
    </div>
