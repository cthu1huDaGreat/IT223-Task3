  ...
  <div class="page-content">
    ...
    <div class="container-fluid p-5">
      <div class="function-card">
        <h2><i class="bi bi-gear me-2"></i>BIN() Function</h2>
        
        <h3>Description</h3>
        <p>The <code>BIN()</code> function converts a decimal number to its binary string representation.</p>
        ...
        <ul style="color: #e6eef3;">
          <li>A string representing the binary value of the number</li>
      ...
      <div class="function-card">
        ...
        <table class="table example-table">
          ...
          <tbody>
            ...
            $examples = [5, 10, 255, 1024];
            foreach ($examples as $num) {
              $result = $conn->query("SELECT BIN({$num}) as binary_value");
              $row = $result->fetch_assoc();
              echo "<tr>";
              echo "<td>{$num}</td>";
              echo "<td><code>" . htmlspecialchars($row['binary_value']) . "</code></td>";
              echo "</tr>";
            }
            ?>
          </tbody>
      ...
      <div class="function-card">
        ...
        <table class="table example-table">
          ...
          <tbody>
            <?php
            $result = $conn->query("SELECT ID, BIN(ID) as binary_id FROM employee LIMIT 5");
            while ($row = $result->fetch_assoc()) {
              echo "<tr>";
              echo "<td>" . htmlspecialchars($row['ID']) . "</td>";
              echo "<td><code>" . htmlspecialchars($row['binary_id']) . "</code></td>";
              echo "</tr>";
            }
            ?>
          </tbody>
      ...
      <div class="function-card">
        ...
        <form method="POST" class="try-form">
          <div class="input-group mb-3">
            <input type="number" class="form-control" name="bin_input" placeholder="Enter a decimal number" value="<?php echo isset($_POST['bin_input']) ? htmlspecialchars($_POST['bin_input']) : ''; ?>" required>
            <button class="btn" type="submit">Convert</button>
          </div>
        </form>
        
        <?php
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['bin_input'])) {
            $input = intval($_POST['bin_input']);
            $result = $conn->query("SELECT BIN({$input}) as binary_value");
            $row = $result->fetch_assoc();
            ?>
            <div class="result-box">
              <strong>Result:</strong> BIN(<?php echo $input; ?>) = <code><?php echo htmlspecialchars($row['binary_value']); ?></code>
            </div>
            <?php
        }
        $conn->close();
