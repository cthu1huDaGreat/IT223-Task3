  ...
  <a class="nav-link py-1" href="../string/upper.php">UPPER</a>
    ...
    <div class="container-fluid p-5">
      <div class="function-card">
        <h2><i class="bi bi-gear me-2"></i>COALESCE Function</h2>
        <h3>Description</h3>
        <p>COALESCE returns the first non-NULL value in a list. Useful for handling NULL values in data.</p>
        <h3>Syntax</h3>
        <pre><code>COALESCE(expr1, expr2, ..., exprN)</code></pre>
        <h3>Returns</h3>
        <ul style="color: #e6eef3;"><li>The first non-NULL expression value</li><li>NULL if all expressions are NULL</li></ul>
      </div>
      <div class="function-card">
        <h3>Database Examples</h3>
        <table class="table example-table">
          <thead><tr><th>Employee ID</th><th>Name</th><th>Department</th><th>Result</th></tr></thead>
          <tbody>
            <?php include "../../config.php"; $result = $conn->query("SELECT ID, Name, Department, COALESCE(Department, 'No Department') as coalesce_result FROM employee LIMIT 5"); while ($row = $result->fetch_assoc()) { echo "<tr><td>" . htmlspecialchars($row["ID"]) . "</td><td>" . htmlspecialchars($row["Name"]) . "</td><td>" . htmlspecialchars($row["Department"]) . "</td><td><code>" . htmlspecialchars($row["coalesce_result"]) . "</code></td></tr>"; } $conn->close(); ?>
          </tbody>
        </table>
      </div>
    </div>
