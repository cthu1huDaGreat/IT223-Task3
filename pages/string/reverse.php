...
<div class="page-content">
  <div class="function-card">
    <h2><i class="bi bi-type me-2"></i>REVERSE Function</h2>
    <h3>Description</h3>
    <p>The <code>REVERSE()</code> function returns a string in reverse order.</p>
    <h3>Syntax</h3>
    <pre><code>REVERSE(string)</code></pre>
    <h3>Returns</h3>
    <ul style="color: #e6eef3;">
      <li>A new string with characters in reverse order</li>
    </ul>
  </div>
  ...
  <div class="function-card">
    ...
    <table class="table example-table">
      ...
      <tbody>
        <?php
          include "../../config.php";
              $sql = "SELECT ID, Name, Department, REVERSE(Name) AS reversed_value FROM employee LIMIT 20";
              $result = $conn->query($sql);
              if ($result) {
                while ($row = $result->fetch_assoc()) {
                  echo "<tr>";
                  echo "<td><code>" . htmlspecialchars($row['Name']) . "</code></td>";
                  echo "<td><code>" . htmlspecialchars($row['reversed_value']) . "</code></td>";
                  echo "</tr>";
                }
                $result->free();
              } else {
