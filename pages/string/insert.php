...
<div class="page-content">
  <div class="function-card">
    <h2><i class="bi bi-type me-2"></i>INSERT Function</h2>
    <h3>Description</h3>
    <p>The <code>INSERT()</code> function inserts a string into another string at a specified position.</p>
    <h3>Syntax</h3>
    <pre><code>INSERT(string, position, length, new_string)</code></pre>
    <h3>Returns</h3>
    <ul style="color: #e6eef3;">
      <li>A new string with the inserted substring</li>
    </ul>
  </div>
  ...
  <div class="function-card">
    ...
    <table class="table example-table">
      ...
      <tbody>
        <?php
          include "../../config.php";
              $sql = "SELECT ID, Name, Department, INSERT(Name, 2, 3, '***') AS inserted_string FROM employee LIMIT 20";
              $result = $conn->query($sql);
              if ($result) {
                while ($row = $result->fetch_assoc()) {
                  echo "<tr>";
                  echo "<td><code>" . htmlspecialchars($row['Name']) . "</code></td>";
                  echo "<td><code>" . htmlspecialchars($row['inserted_string']) . "</code></td>";
                  echo "</tr>";
                }
                $result->free();
              } else {
