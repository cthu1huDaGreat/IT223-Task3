...
<body>
  ...
  <div class="function-card">
    <h2><i class="bi bi-type me-2"></i>REPEAT Function</h2>
    <h3>Description</h3>
    <p>The <code>REPEAT()</code> function returns a string repeated a specified number of times.</p>
    <h3>Syntax</h3>
    <pre><code>REPEAT(string, count)</code></pre>
    <h3>Returns</h3>
    <ul style="color: #e6eef3;">
      <li>String repeated the specified number of times</li>
    </ul>
  </div>
  ...
  <div class="function-card">
    ...
    <table class="table example-table">
      ...
      <tbody>
        <?php
          include "../../config.php";
              $sql = "SELECT ID, Name, Department, REPEAT(Name, 3) AS repeated_value FROM employee LIMIT 20";
              $result = $conn->query($sql);
              if ($result) {
                while ($row = $result->fetch_assoc()) {
                  echo "<tr>";
                  echo "<td><code>" . htmlspecialchars($row['Name']) . "</code></td>";
                  echo "<td><code>" . htmlspecialchars($row['repeated_value']) . "</code></td>";
                  echo "</tr>";
                }
                $result->free();
              } else {
