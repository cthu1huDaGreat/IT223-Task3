...
<div class="page-content">
  <div class="function-card">
    <h2><i class="bi bi-type me-2"></i>CONCAT_WS Function</h2>
    <h3>Description</h3>
    <p>The <code>CONCAT_WS()</code> function concatenates strings with a separator.</p>
    <h3>Syntax</h3>
    <pre><code>CONCAT_WS(separator, string1, string2, ...)</code></pre>
    <h3>Returns</h3>
    <ul style="color: #e6eef3;">
      <li>A string that is the concatenation of all the input strings, separated by the specified separator</li>
    </ul>
  </div>
    ...
    <table class="table example-table">
      <thead>
        <tr>
          ...
          <th>Name</th>
          <th>Department</th>
          <th>CONCAT_WS Result</th>
        </tr>
      </thead>
      <tbody>
        <?php
          include "../../config.php";
              $sql = "SELECT ID, Name, Department, CONCAT_WS(' - ', Name, Department) AS concat_ws_result FROM employee LIMIT 20";
              ...
              if ($result) {
                while ($row = $result->fetch_assoc()) {
                  ...
                  echo "<td><code>" . htmlspecialchars($row['Department']) . "</code></td>";
                  echo "<td><code>" . htmlspecialchars($row['concat_ws_result']) . "</code></td>";
                  echo "</tr>";
                }
                $result->free();
              } else {
